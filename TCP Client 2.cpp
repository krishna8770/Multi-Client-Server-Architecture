...
int main() {
	...
	std::cout << "TCP Client 2 trying to Connect to server" << std::endl;
	...
	std::cout << "TCP Client 2 Connected to server successfully" << std::endl;
	...
	if (getsockname(clientSocket, (sockaddr*)&clientAddr, &addrSize) == SOCKET_ERROR) {
		...
		printf("TCP Client 2 has IP Address: %s and Port Number: %d\n", clientIP, clientPort);
	}

	string fileName = "CancelledChequeRecieved.png";
	string filePath = "C:/Users/knarwani/PracticeProjects/TCP Client 2/Recieve File/" + fileName;
